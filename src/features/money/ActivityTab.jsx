...
import { useExpenses, useCategories, useExpenseMutations } from '../../lib/queries';
...
export function ActivityTab() {
  ...
  async function handleDelete(id) {
    ...
    try { await remove.mutateAsync(id); toast('Deleted'); } catch (err) { toast(err.message, 'error'); }
  }
  ...
  return (
    <div>
              ...
              <div className="txn-list">
                {txns.map(txn => (
                  <div key={txn.id} className="txn" onClick={() => setEditingExpense(txn)}>
                    <div className="txn-ico">{txn.type === 'income' ? '💰' : txn.category?.icon || '💸'}</div>
                    <div className="txn-mid">
                      <div className="txn-name">{txn.note || txn.category?.name || (txn.type === 'income' ? 'Income' : 'Expense')}</div>
      ...
      {editingExpense && (
        <AddEntrySheet initialData={editingExpense} onClose={() => setEditingExpense(null)} />
      )}
    </div>
  );
}
